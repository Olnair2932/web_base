#!/bin/bash
# NEXUS ESTOQUE v2.0 - Automação com Dark Mode
...
NOME=$1; PRECO=$2; IMG=$3; ID=$(date +%s)
TMP=$(mktemp)
...
<head>
    ...
    <style>
        ...
        body { font-family: 'Segoe UI', Arial, sans-serif; margin: 0; background: var(--bg); color: var(--text); padding-bottom: 120px; transition: 0.3s; }
        ...
        .card { background: var(--card); width: 320px; border-radius: 20px; box-shadow: 0 10px 20px rgba(0,0,0,0.05); padding: 15px; text-align: center; color: var(--text); }
